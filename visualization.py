...
model_dict = torch.load('data/METR_LA/LA.tar',map_location=torch.device('cpu'))
...
def visualize_geo(listAss,index,df):
    ...
    save_path = 'data/METR_LA/geo15/'
    ensure_dir(save_path)
    save_name = 'data/METR_LA/geo15/'+"%s"%index + '.json'
...
np.save('data/METR_LA/METR_LA.adjr',adj_mx_r)
for i in notMap:
    map.append([i])
##
